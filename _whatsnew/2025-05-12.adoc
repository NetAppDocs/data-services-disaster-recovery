...
Upon failover, NetApp Disaster Recovery replaces the Network portion of the original IP address (10.1.1) and keeps the host address (.50) of the original IP address (10.1.1.50). For VM1, NetApp Disaster Recovery looks at the CIDR settings for NetworkB and uses the NetworkB network portion, 192.168.1, while keeping the host portion (.50) to create the new IP address for VM1. The new IP becomes 192.168.1.50. 
...
For details about skipping protection, refer to https://docs.netapp.com/us-en/data-services-disaster-recovery/use/failover.html[Fail over applications to a remote site].
