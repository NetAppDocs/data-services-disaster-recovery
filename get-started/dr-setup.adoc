...
[.lead]
To use NetApp Disaster Recovery, complete a few setup steps in the NetApp Console and in your supported disaster recovery environment. 
...
* Hybrid cloud DR that replicates an on-premises VMware plus ONTAP datacenter to an AWS DR infrastructure based on VMware Cloud on AWS or Elastic VMware Service (EVS) and Amazon FSx for NetApp ONTAP.
...
* Hybrid cloud DR that replicates an on-premises VMware plus ONTAP datacenter to a Google DR infrastructure based on Google Cloud VMware Engine and Google Cloud NetApp Volumes.
...
*BEST PRACTICE*: For DR solutions that use VMware Cloud and a single FSx for ONTAP VPC, NetApp recommends using VPC peering. If multiple FSx for ONTAP VPCs are deployed, then we recommend using an AWS Transit Gateway to reduce the management overhead of multiple VPC peer connections.
