...
[.lead]
To use NetApp Disaster Recovery, first prepare your recovery environment before a disruption so replication and failover can be configured and tested with minimal delays.
...
* Hybrid cloud disaster recovery that replicates an on-premises VMware plus ONTAP datacenter to an AWS disaster recovery infrastructure based on VMware Cloud on AWS or Elastic VMware Service (EVS) and Amazon FSx for NetApp ONTAP.
* Private cloud disaster recovery that replicates an on-premises VMware plus ONTAP vCenter to another on-premises VMware plus ONTAP vCenter.
* Cloud disaster recovery that replicates an AWS disaster recovery infrastructure based on VMware Cloud on AWS or EVS to another AWS-based disaster recovery infrastructure using FSx for NetApp ONTAP.
* Hybrid cloud disaster recovery that replicates an on-premises VMware plus ONTAP datacenter to a Google disaster recovery infrastructure based on Google Cloud VMware Engine and Google Cloud NetApp Volumes.

== Hybrid cloud with VMware Cloud and Amazon FSx for NetApp ONTAP 

This method consists of an on-premises production vCenter infrastructure using datastores hosted on ONTAP FlexVol volumes using an NFS protocol. The disaster recovery site consists of one or more VMware Cloud SDDC instances using datastores hosted on FlexVol volumes provided by one or more FSx for ONTAP instances using an NFS protocol. 

The production and disaster recovery sites are connected by an AWS-compatible secure connection. Common connection types are a secure VPN (private or AWS provided), AWS Direct Connect, or other approved interconnect methods. 
...
AWS supports deploying solutions in one or more Availability Zones (AZs) within a region. NetApp Disaster Recovery uses two AWS-hosted services: VMware Cloud on AWS and Amazon FSx for NetApp ONTAP.

* *VMware Cloud for AWS*: Supports deployment in a single-AZ or dual-AZ stretch-cluster SDDC environment. NetApp Disaster Recovery supports only single-AZ SDDC deployments for VMware Cloud on AWS.

* *AWS FSx for NetApp ONTAP*: When Amazon FSx for NetApp ONTAP is deployed in a multi-AZ configuration, each volume is owned by one FSx for ONTAP file system, and the data is mirrored to the second file system. FSx for ONTAP can be deployed in single-AZ or multi-AZ configurations. NetApp Disaster Recovery supports both deployment models.

*BEST PRACTICE*: For AWS disaster recovery site configuration, NetApp recommends using single-AZ deployments for both VMware Cloud and AWS FSx for ONTAP instances. Because AWS is being used for disaster recovery, there is no advantage to introducing multiple Availability Zones. Multi-AZ deployments can increase cost and complexity. 
...
* *AWS Site-to-Site VPN*: This virtual private network connection can provide secure connectivity over a public internet service provider. The VPN encrypts and decrypts all data traveling to and from the AWS cloud. VPNs can be either software- or hardware-based. For enterprise applications, the public internet service provider (ISP) should offer quality-of-service guarantees to ensure that adequate bandwidth and latency are provided for disaster recovery replication. 

*BEST PRACTICE*: For AWS disaster recovery site configuration, NetApp recommends using AWS Direct Connect. This solution provides the highest performance and security for enterprise applications. If it is not available, a high-performance public ISP connection along with a VPN should be used. Ensure that the ISP offers commercial QoS service levels to ensure adequate network performance. 
...
*BEST PRACTICE*: For disaster recovery solutions that use VMware Cloud and a single FSx for ONTAP VPC, NetApp recommends using VPC peering. If multiple FSx for ONTAP VPCs are deployed, then we recommend using an AWS Transit Gateway to reduce the management overhead of multiple VPC peer connections.
...
* Create or verify your destination ONTAP SVM in the AWS FSx for ONTAP instance.
...
Disaster Recovery supports site-to-site replication using Ethernet and TCP/IP. Ensure that adequate bandwidth is available to support data change rates on the production site VMs so that all changes can be replicated to the disaster recovery site within the Recovery Point Objective (RPO) time frame.
...
NetApp Disaster Recovery proxy implementation requires port 443 communication between the Console agent and any vCenter Servers and ONTAP arrays using an HTTPS protocol. The NetApp Disaster Recovery agent within the Console agent communicates directly with VMware vSphere, the vCenter Server, or ONTAP when performing actions.
