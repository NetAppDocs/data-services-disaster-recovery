...
[.lead]
To use NetApp Disaster Recovery, perform a few steps to set it up both in the NetApp Console and the supported cloud environment. 
...
* Hybrid cloud DR that replicates an on-premises VMware plus ONTAP datacenter to an AWS DR infrastructure based on VMware Cloud on AWS or Elastic VMware Service and Amazon FSx for NetApp ONTAP.
...
* Cloud DR that replicates an AWS DR infrastructure based on VMware Cloud on AWS or Elastic VMware Service to another AWS based DR infrastructure using FSx for NetApp ONTAP.
