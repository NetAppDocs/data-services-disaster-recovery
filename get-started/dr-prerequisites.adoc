...
[.lead]
Before using NetApp Disaster Recovery, ensure your environment meets the ONTAP storage, VMware vCenter cluster, and NetApp Console requirements. Validating the prerequisites ensures your environment is ready to replicate and recover workloads without delays.

== Software versions

[options="header",cols="35,55a",width="100%"]
|===
| Component
| Minimum version
...
When you use NetApp Disaster Recovery with Google Cloud VMware Engine and Google Cloud NetApp Volumes, ensure that you configure the correct permissions and review the following considerations. 

* Contact the Google SRE team to allow-list the following:
...
These prerequisites apply to ONTAP and Amazon FSx for NetApp ONTAP instances.
...
* link:task-kubernetes-requirements.html[Review the additional requirements for Kubernetes clusters].
...
If you haven't already done so, https://docs.netapp.com/us-en/console-setup-admin/task-sign-up-saas.html[sign up for the NetApp Console and create an organization^].
...
[TIP]
To use Disaster Recovery with Amazon Elastic VMware Service (EVS), see the link:../reference/evs-deploy-guide-config-prereqs.html[additional prerequisites and configuration guidance^]. 
...
[IMPORTANT]
Depending on your deployment type, review the link:reference-vmware-prerequisites.html[VMware cluster prerequisites] and link:task-kubernetes-requirements.html[Kubernetes cluster prerequisites].

== More information

* xref:../reference/vcenter-privileges.html[Required vCenter privileges]
* xref:../reference/evs-deploy-guide-config-prereqs.html[Prerequisites for Amazon EVS with NetApp Disaster Recovery]
