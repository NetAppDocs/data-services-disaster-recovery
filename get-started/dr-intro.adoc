...
[.lead]
NetApp Disaster Recovery is a cloud-based service that automates disaster recovery workflows for supported on-premises VMware and Kubernetes workloads running on NetApp storage. You can protect these workloads to supported cloud or on-premises disaster recovery targets. 
...
NetApp Disaster Recovery uses ONTAP SnapMirror technology and native VMware orchestration to protect VMware VMs and their on-disk OS images while retaining ONTAP storage efficiency benefits. SnapMirror provides the replication transport to the disaster recovery site. This enables industry-best storage efficiency (compression and deduplication) on primary and secondary sites. 
