...
[.lead]
After you've added resources and sites, you're ready to create a _replication plan_. Replication plans manage the data protection. Creating the replication involves designating the source and destination sites, selecting the resources groups, and choosing how applications should be restored and powered on. For example, you might group virtual machines (VMs) associated with one application or you might group applications that have similar tiers. 
...
[TIP]
Organize your VMs or Kubernetes clusters before deploying NetApp Disaster Recovery to minimize sprawl. Place resources that need protection on a subset of datastores and place resources that are not going to be protected on a different subset of datastores. To organize resources groups before creating the replication plan, see link:resource-group.html[Create a resource group].
...
. For *Source*, select the vCenter or Kubernetes cluster to desginate as the source. 
...
.Steps
. If you have existing resource groups, choose the groups from the list then.
+
