...
* Ensure there's at least one VM in the datastore. If there are no VMs in the datastore, the datastore won't discovered.
...
.Steps
. If you have existing resource groups, chooes the groups from the list then.
+
...
+
. In the alternate column, review the selected resources groups.  
. Select **Next**. 
--
====
...
[NOTE]
For Kubernetes, this is the final step for creating a replication. 
+
For vCenters, continue to the <<Compute resources section>>.
...
* *CPU* and *Memory (GB)*: Under the virtual machines details, you can optionally resize the VM CPU and . 
+
