...
[.lead]
When a disaster or over other similar event occurs, use NetApp Disaster Recovery to recover critical workloads so applications can resume at the disaster recovery site with planned mappings.

Disaster Recovery also supports testing failovers. Performing tests helps familiarize you with the process and confirms that recovery works as you expect. 
...
During a failover test, Disaster Recovery temporarily creates VMs. Disaster Recovery maps a temporary datastore backing the FlexClone volume to the ESXi hosts.
...
* Register the target VMs within vCenter as new datastores.
...
For datastore-based replication plans, if you've added and discovered any VMs but not provided mapping details, those VMs are included in the failover. The failover will fail with a notification in the jobs. You must provide the mapping details to complete the failover successfully.  
...
. Choose the snapshot copy you want to use for failover:
+
* Choose *Take snapshot now* to create a brand new snapshot that will be copied to the destination cluster before failover. 
* Choose *Use latest snapshot* to use the most recent snapshot available on the destination cluster. 
* Choose *Select* to review a list of snapshots sorted by recency. 
+
include::../_include/latest-snapshot.adoc[]
...
. Choose the snapshot copy you want to use for failover:
+
* Choose *Take snapshot now* to create a brand new snapshot that will be copied to the destination cluster.
* Choose *Use latest snapshot* to use the most recent snapshot available on the destination cluster. 
* Choose *Select* to review a list of snapshots sorted by recency. 
+
include::../_include/latest-snapshot.adoc[]
. Select *Skip protection* if you want Disaster Recovery not to automatically create a reverse SnapMirror protection relationship after a replication plan failover. This is useful if you want to perform additional operations on the restored site before you bring it back online within NetApp Disaster Recovery. 
