...
Before conducting a live failover, you can perform a test to ensure the readiness of your workloads. Workloads remain online during the test process. 
...
. On the right, select the *Actions* option image:../use/icon-horizontal-dots.png[Actions menu icon in NetApp Disaster Recovery service], hover over **Test plan** then select *Test failover*.
...
+
Enter "Test failover" and select *Test fail over*.  
. After the test is complete, clean up the test environment.

[NOTE]
When you finish the test, it's recommended you reset the VMs with the *Clean up test* option. 

=== Clean up after a failover test
...
. On the right, select the *Actions* option image:../use/icon-horizontal-dots.png[Actions menu icon in NetApp Disaster Recovery service], hover over **Test failover** then *Clean up failover test*.

. In the Test failover page, enter "Clean up test failover" in the text box then select *Clean up test failover test*.  
...
[IMPORTANT]
If the replication plan includes an empty datastore, you must first remove the empty datastore from the plan or use the **Force failover** option, which skips the empty datastores. 
...
. Select *Force failover* if you want the failover to occur even if an error is detected that would normally prevent the failover from occurring. 
