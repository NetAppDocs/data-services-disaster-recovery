...
During a failover test, virtual machines are temporarily created. NetApp Disaster Recovery does not map the target volume. Instead, it makes a new FlexClone volume from the selected snapshot, and a temporary datastore backing the FlexClone volume is mapped to the ESXi hosts.
