...
summary: Resource groups let you organize dependent resources into logical groups that meet your requirements.
---
= Create a resource group in NetApp Disaster Recovery
:hardbreaks:
:icons: font
:imagesdir: ../media/use/

[.lead]
After you add resources to sites, you can create resource groups to organize VMs, datastores, or Kubernetes clusters as a single unit. Resource groups let you organize dependent resources into logical groups that meet your requirements. For example, you can group VMs for one application or group applications that share similar tiers. Resource groups can also include delayed boot orders that run during recovery to ensure resources start in the correct order.
...
TIP: Each resource group can include one or more VMs, datastores, or Kubernetes clusters. The VMs will power on based on the sequence in which you include them in the replication plan. You can change the order by dragging the VMs or datastores up or down the resource group list. 

.About resource groups

Resource groups let you combine VMs, datastores, or Kubernetes applications into a single unit. 
...
NetApp Disaster Recovery protects VMs by replicating the underlying ONTAP volumes and LUNs hosting the VMs in the resource group. To do this, the system queries vCenter for the name of each datastore hosting VMs in a resource group. NetApp Disaster Recovery then identifies the source ONTAP volume or LUN hosting that data store. All protection is performed at the ONTAP volume level using SnapMirror replication.

If VMs in the resource group are hosted on different datastores, NetApp Disaster Recovery uses one of the following methods to create a data-consistent snapshot of the ONTAP volumes or LUNs.
...
| * ONTAP API: `cg_start`. Quiesces all volumes so snapshots can be taken and initiates volume-scoped snapshots of all resource group volumes.
...
. In Disaster Recovery, select **Resource groups** in the sidebar. 
. In the dropdown menu, filter by *VMware*.
+
...
.Steps
. In Disaster Recovery, select **Resource groups** in the sidebar. 
. In the dropdown menu, filter by *Kubernetes*.
+
image::site-type-toggle.png[Screenshot of the site type toggle]
. Select *Add*. 
. Enter a **Name** for the resource group and select the Kubernetes **Cluster** from the dropdown menu.
. Select the *Namespaces* to add to the resource group.
. Classify the *Resource types*. By default, the selection is "Includes all," which denotes multiple resource types.
. Optionally, include any *Label selectors*. 
+
image:kubernetes-resource-group.png[Screenshot of add a Kubernetes resource group]
. Optionally, select *+ Cluster-scoped resources* to add resources that aren't scoped to a namespace. If you choose this option, select the *Group*, *Version*, *Kind*, and optionally add any label selectors. Select **Add resource**.
+
image:kubernetes-cluster-scoped.png[Screenshot of add Cluster-scoped Kubernetes resources]
. Select **Add**. 
--
====
