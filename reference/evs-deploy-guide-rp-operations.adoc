...
[.lead]
After configuring the replication plan in NetApp Disaster Recovery, you can perform failovers, test failover, refresh resources, migrate, and manage snapshots and replications plans. The primary action to perform, beyond maintenance, is failover in the event of a failure.
...
. Choose which snapshot to use. To create the most up-to-date copy of the production environment, choose *Take snapshot now*. To use a previously captured snapshot, choose *Select* then pick the existing snapshot.
...
Any time you make changes to your virtual infrastructure—such as adding or deleting VMs, adding or deleting datastores, or moving VMs between datastores—you need to perform a refresh of the impacted vCenter clusters in Disaster Recovery. Disaster Recovery refreshes automatically once every 24 hours by default, but a manual refresh ensures that the latest virtual infrastructure information is available and taken into account for disaster recovery protection.
...
In addition to using NetApp Disaster Recovery to manage replication and failover in the face of disasters, it can also support one-time VM migrations from the source site to the destination site. You might perform a one-time migration as part of a move to the cloud or a disaster avoidance procedure when responding to catastrophic events such as extreme weather or political turmoil.
...
Because Disaster Recovery orchestrates ONTAP volume snapshots, an ONTAP storage administrator can directly delete snapshots using either ONTAP System Manager, the ONTAP CLI, or the ONTAP REST APIs without the service's knowledge. Disaster Recovery  automatically deletes any snapshots on the source that are not on the destination cluster automatically every 24 hours. You can delete snapshots manually as needed. This feature enables you to ensure that the snapshots are consistent across all sites.
