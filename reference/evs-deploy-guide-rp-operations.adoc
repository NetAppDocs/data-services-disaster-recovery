...
Any time you make changes to your virtual infrastructure—such as adding or deleting VMs, adding or deleting datastores, or moving VMs between datastores—you need to perform a refresh of the impacted vCenter clusters in the Disaster Recovery service. The service does this automatically once every 24 hours by default, but a manual refresh ensures that the latest virtual infrastructure information is available and taken into account for disaster recovery protection. 
