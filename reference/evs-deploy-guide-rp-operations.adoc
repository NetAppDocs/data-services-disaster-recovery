...
[.lead]
Use this page to perform replication plan operations in NetApp Disaster Recovery, including failover, test failover, migration, snapshot management, and schedule updates for Amazon EVS environments.

== Fail over

The primary operation is failover after a catastrophic failure at the production on-premises site.
...
. Decide whether to capture the latest state of the production environment (Take snapshot now) or use a previously created replication plan backup (Select)
...
Any time you make changes to your virtual infrastructure -- such as adding or deleting VMs, adding or deleting datastores, or moving VMs between datastores -- you need to perform a refresh of the impacted vCenter clusters in the NetApp Disaster Recovery service. The service does this automatically once every 24 hours by default, but a manual refresh ensures that the latest virtual infrastructure information is available and taken into account for disaster recovery protection. 
