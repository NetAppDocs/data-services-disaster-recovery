...
[.lead]
Configure the required ONTAP permissions so NetApp Disaster Recovery can function properly. 

== Minimum ONTAP permissions

NetApp Disaster Recovery requires the following ONTAP permissions:
...
To add permissions in ONTAP, create a role with the required privileges and assign it to a user. You can complete this task in ONTAP by using System Manager or the ONTAP CLI. 
...
. Return to the *Users and roles* section. 
. In *Users*, select *Add*. 
...
.CLI
--
To create a role and user in ONTAP, sign in to the appropriate ONTAP cluster CLI with administrator credentials. You must perform this process separately for the source and destination clusters. 

.Create the role with required privileges
. Run the `security login rest-role create -role <rolename> -api <api> -access <access>` command to create a role with the required privileges. You can copy the following commands into a script to create the `dr_privileges` role.
+
----
...
After creating the ONTAP roles, you need to link:https://docs.netapp.com/us-en/storage-management-ontap-onprem/task-discovering-ontap.html#step-3-discover-a-cluster[discover the ONTAP cluster in the Console^]. When you discover the cluster, provide the ONTAP cluster IP address and the credentials for the user you created. For Disaster Recovery, you must perform discovery on both the source and destination cluster. 