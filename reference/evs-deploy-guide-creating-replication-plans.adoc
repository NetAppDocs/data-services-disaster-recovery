...
* The section marked with the check icon (image:evs-check-icon.png[20,20]) have been mapped or have default mappings. Review them to make sure that the current configuration meets your requirements.

.Access resouce mapping
...
Disaster Recovery supports an ordered restart of VMs based on a boot order field. The oot order field indicates how the VMs in each resource group start. Those VMs with the same value in the boot order field boot in parallel.
...
. Optoinally, check the *Create application consistent replicas* checkbox if the VM is hosting an Oracle or SQL Server database.
