...
[.lead]
The vCenter account for NetApp Disaster Recovery must have a minimum set of privileges to register and deregister datastores, start and stop virtual machines (VMs), and reconfigure VMs.

The following table lists the privileges that NetApp Disaster Recovery requires to work with a vCenter cluster.
...
| Allows scanning of new storage devices and management of iSCSI.

| Resource 
| Resource.AssignVMToPool
| Resource pools
| Allows VM assignment to a resource pool 
...
.12+| Virtual machine configuration
...
| Add existing disk
| Allows adding an existing virtual disk to a VM.

| VirtualMachine.Config.AddNewDisk
| Add new disk
| Allows creating a new virtual disk to add to a VM.
...
| Change resource
| Permits changing the resource configuration of VMs in a resource pool.
